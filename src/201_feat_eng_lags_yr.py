# %%
import pandas as pd
from src.utils.preprocessing import add_basic_valid_lag_features
...
total_data_with_lags = (
    total_data.groupby("code", as_index=False).progress_apply(
        lambda x: add_basic_valid_lag_features(x, 14, 3, 1)
    )
).reset_index()
# %%
...
total_data_with_lags.query('date<"01-01-2022"').to_parquet(
    "data/201_feateng_train_data.parquet"
)
total_data_with_lags.query('date>="01-01-2022"').to_parquet(
    "data/201_feateng_test_data.parquet"
)
