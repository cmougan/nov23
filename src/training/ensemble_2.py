...
ensemble_df["inverse_error"] = 0.001 + (ensemble_df["error"].max() / ensemble_df["error"]) - 1
...
# %%
ensemble_df.drop(columns=["inverse_error"]).to_csv(submission_path / "submission_ensemble_102.csv", index=False)
# %%
