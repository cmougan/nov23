...
# %%
train_data = pd.read_parquet("data/200_feateng_train_data.parquet").drop(columns = ['level_0','level_1'])
print(train_data.isna().sum() / len(train_data))

# %%
submission_data = pd.read_parquet("data/200_feateng_test_data.parquet").drop(columns = ['level_0','level_1'])
...
X_non_cat = X.drop(columns = ["ther_area",
    ...
    "country",
    "hospital_rate"])
X_non_cat = X_non_cat.fillna(method = 'bfill').astype(float)
X_non_cat = X_non_cat.fillna(method = 'ffill').astype(float)
#%%
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X_non_cat)
# %%
lasso = Lasso(alpha = 0.1)
lasso.fit(X_non_cat,y)
# %%
