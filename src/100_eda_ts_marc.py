...
train_data['code'] = ['_'.join([str(brand),str(country)]) for brand,country in zip(train_data['brand'],train_data['country']) ]
...
submission_data['code'] = ['_'.join([str(brand),str(country)]) for brand,country in zip(submission_data['brand'],submission_data['country']) ]
...
daily_info = train_data.groupby('date',as_index = False).size() #not  all days contain all the info.
...
monthly_accum_sales = train_data.groupby('date',as_index = False).monthly.mean() #not sum since not all days contain all the info.
...
evolution_phase_per_year = train_data.groupby(['year','wd'],as_index = False).phase.mean()
