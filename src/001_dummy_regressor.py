...
train_data_tr, train_data_te = train_test_split(train_data, test_size=0.2, random_state=42)

# %%
prediction_day_map = train_data_tr.groupby('dayweek',as_index = False).phase.mean().rename(columns = {'phase':'prediction'})
...
# %%
submission_data_with_prediction = get_dummy_prediction(submission_data, prediction_day_map)[submission.keys()]
